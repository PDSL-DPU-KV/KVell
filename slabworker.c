...
struct slab_context {
  ...
  volatile size_t processed_callbacks; // Number of requests fully submitted and
                                       // processed on disk
...
again:
  for (size_t i = 0; i < pending; i++) {
    ...
    case ADD:
      if (e) {
        die("Adding item that is already in the database! Use update instead! "
            "(This error might also appear if 2 keys have the same prefix, "
            "TODO: make index more robust to that.)\n");
      } else {
    ...
    case UPDATE:
      if (!e) {
        ...
        assert(
            ...
            e->slab->item_size); // Item grew, this is not supported currently!
...
static void worker_slab_init_cb(struct slab_callback *cb, void *item) {
  ...
  if (!memory_index_lookup(get_worker(cb->slab), item)) {
    ...
    /* Complex path -- item is already in the index, we should decide which one
     * to keep based on rdt! */
    printf("#WARNING! Item is present twice in the database! Has the database "
           "crashed?\n");
