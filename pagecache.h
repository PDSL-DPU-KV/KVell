...
#define tree_lookup(h, hash)                                                   \
  ({                                                                           \
    void *__v = raxFind((h), (unsigned char *)&(hash), sizeof(hash));          \
    __v == raxNotFound ? NULL : __v;                                           \
  })
#define tree_delete(h, hash, old_entry)                                        \
  raxRemove((h), (unsigned char *)&(hash), sizeof(hash), (void **)(old_entry))
#define tree_insert(h, hash, old_entry, dst, lru_entry)                        \
  do {                                                                         \
...
#define tree_create()                                                          \
  ({                                                                           \
    art_tree *___t = malloc(sizeof(*___t));                                    \
    art_tree_init(___t);                                                       \
    ___t;                                                                      \
  })
#define tree_lookup(h, hash)                                                   \
  art_search((h), (unsigned char *)&(hash), sizeof(hash))
#define tree_delete(h, hash, old_entry)                                        \
  *old_entry = art_delete((h), (unsigned char *)&(hash), sizeof(hash))
#define tree_insert(h, hash, old_entry, dst, lru_entry)                        \
  do {                                                                         \
    pagecache_entry_t *new_entry = old_entry;                                  \
    if (!new_entry)                                                            \
