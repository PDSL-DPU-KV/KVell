  ...
  for (size_t i = 0; i < nb_accesses;) {
    ...
    for (size_t j = 0; j < queue_size; j++) {
      ...
      if (pdata->rw == RO) {
        ...
        cb[j].aio_lio_opcode =
            (rand_r(&seed) % 2) ? IOCB_CMD_PWRITE : IOCB_CMD_PREAD;
      } else {
        cb[j].aio_lio_opcode =
            (rand_r(&seed) % 100 < 5) ? IOCB_CMD_PWRITE : IOCB_CMD_PREAD;
      }
    ...
    __1 ret = io_getevents(ctx, ret, ret, events, NULL);
    __2

        wait_for(450000);
    __3

        show_breakdown_periodic(1000, i, "io_submit", "io_getevents", "waiting",
                                "unused", "unused");
  }
...
int bench_io(void) {
  ...
  } stop_timer("DirectIO - Time for %lu accesses = %lums (%lu io/s)",
  NB_ACCESSES, elapsed/1000, NB_ACCESSES*1000000LU/elapsed);*/
  ...
  for (size_t rw = RO; rw <= RO; rw++) {
    for (size_t q = 0; q < sizeof(queue_sizes) / sizeof(*queue_sizes); q++) {
      ...
      stop_timer("libaio %d threads - %s - Time for %lu accesses queue size "
                 "%lu = %lums (%lu io/s)",
                 NB_THREADS, rw_to_str(rw), NB_ACCESSES, queue_size,
                 elapsed / 1000, NB_ACCESSES * 1000000LU / elapsed);
    }
  }

  /* MMap perf */
  /*start_timer {
     char *map = mmap(NULL, sb.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
  0); for(size_t i = 0; i < NB_ACCESSES; i++) { uint64_t page = xorshf96() %
  nb_pages; memcpy(page_data, &map[page * PAGE_SIZE], PAGE_SIZE);
     }
  } stop_timer("MMAP - Time for %lu accesses = %lums (%lu io/s)", NB_ACCESSES,
  elapsed/1000, NB_ACCESSES*1000000LU/elapsed);*/
...
int bench_data_structures(void) {
  ...
  stop_timer("RBTREE - Time for %lu inserts/replace (%lu inserts/s)",
             NB_INSERTS, NB_INSERTS * 1000000LU / elapsed);
  ...
  stop_timer("RBTREE - Time for %lu finds (%lu finds/s)", NB_INSERTS,
  ...
  stop_timer("RAX - Time for %lu inserts/replace (%lu inserts/s)", NB_INSERTS,
             NB_INSERTS * 1000000LU / elapsed);
  ...
  stop_timer("RAX - Time for %lu finds (%lu finds/s)", NB_INSERTS,
             NB_INSERTS * 1000000LU / elapsed);
  ...
  stop_timer("ART - Time for %lu inserts/replace (%lu inserts/s)", NB_INSERTS,
             NB_INSERTS * 1000000LU / elapsed);
  ...
  stop_timer("ART - Time for %lu finds (%lu finds/s)", NB_INSERTS,
             NB_INSERTS * 1000000LU / elapsed);
  ...
  } stop_timer("HASH - Time for %lu inserts/replace (%lu inserts/s)",
  NB_INSERTS, NB_INSERTS*1000000LU/elapsed);*/
