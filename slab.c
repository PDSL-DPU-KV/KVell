 ...
 */
void add_existing_item(struct slab *s, size_t idx, void *_item,
  ...
  if (item->key_size == -1) { // Removed item
    ...
    if (idx > s->last_item)
      s->last_item = idx;
    if (item->rdt >
        get_rdt(s->ctx)) // Remember the maximum timestamp existing in the DB
...
void process_existing_chunk(int slab_worker_id, struct slab *s, size_t nb_files,
                            size_t file_idx, char *data, size_t start,
                            size_t length, struct slab_callback *callback) {
  ...
  for (size_t p = 0; p < nb_pages; p++) {
    ...
    size_t base_idx =
        page_num * nb_items_per_page * nb_files + file_idx * nb_items_per_page;
    ...
    for (size_t i = 0; i < nb_items_per_page; i++) {
      ...
      current += s->item_size;
      periodic_count(
          1000, "[SLAB WORKER %d] Init - Recovered %lu items, %lu free spots",
          slab_worker_id, s->nb_items, s->nb_free_items);
    }
  }
}

#define GRANULARITY_REBUILD (2 * 1024 * 1024) // We rebuild 2MB by 2MB
void rebuild_index(int slab_worker_id, struct slab *s,
                   struct slab_callback *callback) {
  ...
  while (1) {
    ...
    process_existing_chunk(slab_worker_id, s, 1, 0, cached_data, start,
                           end - start, callback);
    start = end;
  }
  free(cached_data);
...
struct slab *create_slab(struct slab_context *ctx, int slab_worker_id,
                         size_t item_size, struct slab_callback *callback) {
  struct stat sb;
  char path[512];
  ...
  if (meta->key_size !=
      0) { // if the key_size is not 0 then then file has been written before
    callback->slab = s;
...
void update_item_async_cb1(struct slab_callback *callback) {
  ...
  if (callback->action == UPDATE) {
    ...
    if (new_key_size != old_key_size) {
      die("Updating an item, but key size changed! Likely this is because 2 "
          "keys have the same prefix in the index and we got confused because "
          "they have the same prefix. TODO: make the index more robust by "
          "detecting that 2 keys have the same prefix and transforming the "
          "prefix -> slab_idx to prefix -> [ { full key 1, slab_idx1 }, { full "
          "key 2, slab_idx2 } ]\n");
    }
    ...
    if (memcmp(new_key, old_key, new_key_size))
      die("Updating an item, but key mismatch! Likely this is because 2 keys "
          "have the same prefix in the index. TODO: make the index more robust "
          "by detecting that 2 keys have the same prefix and transforming the "
          "prefix -> slab_idx to prefix -> [ { full key 1, slab_idx1 }, { full "
          "key 2, slab_idx2 } ]\n");
  }
...
void add_item_async_cb1(struct slab_callback *callback) {
  ...
  if (lru_entry == NULL) { // no free page, append
    ...
    s->last_item++;
  } else { // reuse a free spot. Don't forget to add the linked tombstone in the
           // freelist.
    char *disk_page = callback->lru_entry->page;
    off_t in_page_offset =
        item_in_page_offset(callback->slab, callback->slab_idx);
    add_son_in_freelist(callback->slab, callback->slab_idx,
                        (void *)(&disk_page[in_page_offset]));
  }
  s->nb_items++;
